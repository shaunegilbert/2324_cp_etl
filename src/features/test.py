...
def pathway_code_pivot(base_dirs):
    ...
    essential_info_cols = ['STUDENT_NUMBER', 'COHORTYR', 'GRADE_LEVEL', 'SCHOOL_NAME', 'EXITDATE', 'pathway_code']
    ...
    df_pivot = df_filtered.pivot_table(index='STUDENT_NUMBER', columns='pathway_code', values='pathway_present', aggfunc='first').reset_index()

    # Ensuring 'STUDENT_NUMBER' is treated consistently
    df_students['STUDENT_NUMBER'] = df_students['STUDENT_NUMBER'].astype(str)
    ...
    pathway_code_pivot = pd.merge(df_students, df_pivot, how='left', on='STUDENT_NUMBER')
    ...
    pathway_code_pivot.to_csv(output_file_path, index=False)  # Consider if you need the index; adjust accordingly
    logging.info(f"Merged DataFrame has been written to {output_file_path}")
