...
def ect_c3_processed():
    try:
        service = get_google_sheets_service()
        spreadsheet_id = os.getenv('ECT_C3_PROCESSED_ID')
        range_name = os.getenv('ECT_C3_PROCESSED_RANGE')
        csv_file=os.path.join('data', 'processed', 'ect_c3_processed.csv')
